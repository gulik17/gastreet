...
class AlbaService {
     ...
     * @return array
     */
    public function initPayment($pay_type, $cost, $name, $email, $phone, $order_id=False, $commission='partner') {
        $fields = array(
            "cost" => $cost,
            "name" => $name,
            "email" => $email,
     ...
     *               'ym' - Яндекс.Деньги
     *               'mtsmoney' - МТС Деньги
     * @param integer $cost - Сумма
     * @param string $name - Описание
     * @param string $email - Ящик
     * @param string $phone - Телефон
     * @param integer $order_id - номер заказа
     * @return string HTML форма оплаты
     */
    public function showPaymentForm($pay_type, $cost, $name, $comment, $email, $phone, $order_id=False, $key=null) {
        if (!$order_id) {
        ...
        $output  = "<form name='paymentform' action='" . static::BASE_URL . "alba/input/' method='POST'>";
    ...
    public function refund($tid, $amount=False, $test=False, $reason=False) {
        ...
        $fields = array("version" => "2.0",'tid' => $tid);
        if ($amount) {
