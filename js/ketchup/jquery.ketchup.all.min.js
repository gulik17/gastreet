...
jQuery.ketchup.validation("required","Поле необходимо заполнить.",function(g,b,c){g=b.attr("type").toLowerCase();return g=="checkbox"||g=="radio"?b.attr("checked")==true:c.length!=0}).validation("minlength","Минимальная длина {arg1}.",function(g,b,c,d){return c.length>=+d}).validation("maxlength","Максимальная длина {arg1}.",function(g,b,c,d){return c.length<=+d}).validation("rangelength","Длина от {arg1} до {arg2}.",function(g,
b,c,d,h){return c.length>=d&&c.length<=h}).validation("min","Ожидается {arg1} или больше.",function(g,b,c,d){return this.isNumber(c)&&+c>=+d}).validation("max","Не может быть более {arg1}.",function(g,b,c,d){return this.isNumber(c)&&+c<=+d}).validation("range","Должно быть от {arg1} до {arg2}.",function(g,b,c,d,h){return this.isNumber(c)&&+c>=+d&&+c<=+h}).validation("number","Ожидается число",function(g,b,c){return this.isNumber(c)}).validation("digits","Ожидается число.",function(g,b,c){return/^\d+$/.test(c)}).validation("email",
"Не верный E-Mail.",function(g,b,c){return this.isEmail(c)}).validation("url","Не верная ссылка URL.",function(g,b,c){return this.isUrl(c)}).validation("username","Есть недопустимые символы.",function(g,b,c){return this.isUsername(c)}).validation("match","Ожидается {arg1}.",function(g,b,c,d){return b.val()==d}).validation("contain","Ожидается {arg1}",function(g,b,c,d){return this.contains(c,d)}).validation("date","Не верная дата.",function(g,b,c){return this.isDate(c)}).validation("minselect",
"Нужно отметить минимум {arg1} вариантов.",function(g,b,c,d){return d<=this.inputsWithName(g,b).filter(":checked").length},function(g,b){this.bindBrothers(g,b)}).validation("maxselect","Можно отметить не более {arg1} вариантов.",function(g,b,c,d){return d>=this.inputsWithName(g,b).filter(":checked").length},function(g,b){this.bindBrothers(g,b)}).validation("rangeselect","Отметьте от {arg1} до {arg2} вариантов.",function(g,b,c,d,h){g=this.inputsWithName(g,b).filter(":checked").length;return d<=g&&h>=
